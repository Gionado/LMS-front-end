...
import { ArrowRight, BookOpen, CalendarDays, CheckCircle2, Clock3, Download, Library, Play } from "lucide-react";
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-xl bg-[#0B2D5C] p-6 text-white shadow-sm sm:p-8">
        ...
        <div className="relative z-10 max-w-2xl">
          <h1 className="text-2xl font-bold tracking-[-.02em] sm:text-3xl">Good morning, Gionado!</h1>
      ...
      <section aria-labelledby="learning-overview">
        <h2 id="learning-overview" className="mb-4 text-lg font-semibold text-[#0B2D5C]">Learning overview</h2>
      ...
      <section>
        <div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold text-[#0B2D5C]">Continue learning</h2><Link href="/courses" className="flex items-center gap-1 text-sm font-medium text-[#155EAA] hover:underline">View all <ArrowRight size={15} /></Link></div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <section className="rounded-xl border border-slate-200 bg-white p-5 sm:p-6">
          <div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-semibold text-[#0B2D5C]">Upcoming schedule</h2><CalendarDays className="text-slate-400" size={20} /></div>
          <div className="space-y-3">{schedule.map((item) => <article key={item.title} className="flex items-center gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-white text-center shadow-sm"><span><span className="block text-sm font-bold leading-4 text-[#0B2D5C]">{item.date}</span><span className="text-[9px] font-semibold text-[#155EAA]">{item.day}</span></span></div><div className="min-w-0"><h3 className="truncate text-sm font-semibold text-slate-800">{item.title}</h3><p className="mt-0.5 truncate text-xs text-slate-500">{item.detail}</p></div><span className="ml-auto shrink-0 text-xs font-semibold text-[#155EAA]">{item.time}</span></article>)}</div>
        </section>

        <section className="rounded-xl border border-slate-200 bg-white p-5 sm:p-6">
          <h2 className="mb-5 text-lg font-semibold text-[#0B2D5C]">Weekly goal</h2>
