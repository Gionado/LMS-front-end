...
import { BookOpen, Construction, FileQuestion, LayoutDashboard, Library, LogOut, Menu, ReceiptText, X } from "lucide-react";
...
const demoNavigation = [
  { label: "Maintenance Page", href: "/maintenance", icon: Construction },
  { label: "404 Error Page", href: "/demo-404", icon: FileQuestion },
];
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const pageLabel = pathname.startsWith("/payment")
    ? "Payment"
    : [...navigation, ...demoNavigation].find((item) => pathname.startsWith(item.href))?.label ?? "Student Portal";
    ...
    <div className="min-h-screen bg-slate-50 text-[#172033]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[272px] flex-col overflow-y-auto bg-[#0B2D5C] px-4 py-5 text-white transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="mt-8 border-t border-white/10 pt-5">
          <p className="px-3.5 text-[10px] font-semibold uppercase tracking-[.16em] text-sky-100/45">Other Demo Pages</p>
          <ul className="mt-2 space-y-1">
            {demoNavigation.map((item) => { const Icon = item.icon; const active = pathname.startsWith(item.href); return <li key={item.href}><Link href={item.href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-medium transition ${active ? "bg-white/10 text-white" : "text-sky-50/70 hover:bg-white/10 hover:text-white"}`}><Icon size={18} /><span>{item.label}</span></Link></li>; })}
          </ul>
        </div>
          ...
          <div className="mx-auto flex max-w-[1376px] items-center gap-3">
            ...
            <div className="relative ml-auto">
              <button type="button" onClick={() => setProfileOpen((value) => !value)} className="flex items-center gap-2 rounded-xl p-1.5 pr-2 transition hover:bg-slate-50" aria-expanded={profileOpen} aria-label="Open profile menu">
                <span className="grid h-8 w-8 place-items-center rounded-full bg-[#F6C667] text-xs font-extrabold text-[#0B2D5C]">NP</span><span className="hidden text-left xl:block"><span className="block text-xs font-bold">Nadira Putri</span><span className="block text-[10px] text-slate-500">Student</span></span>
              </button>
