...
import { BookMarked, CheckCircle2, ChevronDown, Download, Eye, FileText, Search, ShoppingCart, SlidersHorizontal, X } from "lucide-react";
...
export function EbooksView() {
  ...
  const purchased = (product: DemoProduct, nextOwnedKeys: string[]) => {
    ...
    setBuying(null);
    notify(`${product.title} is now owned.`);
  };
  ...
  return (
    <>
          ...
          {filtered.map((ebook) => (
            <article key={ebook.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300">
              ...
              <div className="pt-4">
                ...
                <h3 className="line-clamp-1 text-sm font-semibold text-slate-800">{ebook.title}</h3>
      ...
      <DemoPurchaseModal product={buying ? { id: buying.id, title: buying.title, type: "Ebook", price: buying.price } : null} onClose={() => setBuying(null)} onPurchased={purchased} />
