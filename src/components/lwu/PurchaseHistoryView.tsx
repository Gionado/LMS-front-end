...
import { Download, Eye, PackageCheck, ReceiptText, Search, WalletCards, X } from "lucide-react";
...
export function PurchaseHistoryView() {
  ...
  const successfulTotal = purchases.filter((purchase) => purchase.status === "Successful").reduce((total, purchase) => total + purchase.amount, 0);
  const summary = [
    { label: "Total purchases", value: purchases.length.toString(), icon: WalletCards, color: "bg-sky-50 text-[#155EAA]" },
    { label: "Successful", value: purchases.filter((purchase) => purchase.status === "Successful").length.toString(), icon: PackageCheck, color: "bg-emerald-50 text-emerald-700" },
  ...
  return (
    <>
      ...
      <div className="mt-5 grid gap-3 rounded-xl border border-slate-200 bg-white p-4 md:grid-cols-[minmax(0,1fr)_160px_160px]">
        ...
        <select aria-label="Filter purchase status" value={status} onChange={(event) => setStatus(event.target.value)} className="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-600"><option>All status</option><option>Successful</option><option>Pending</option><option>Refunded</option></select>
        <select aria-label="Filter product type" value={type} onChange={(event) => setType(event.target.value)} className="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-600"><option>All products</option><option>Course</option><option>Ebook</option></select>
      </div>
      ...
      <div className="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <div className="border-t border-slate-100 px-5 py-4 text-xs text-slate-500">{filtered.length} of {purchases.length} transactions</div>
      </div>
